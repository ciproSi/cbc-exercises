...
export default class Pacman {
    ...
    moveRight() {
        ...
        if (entity.type === 'wall') {
            ...
            if (this.stage.width - 85 > this.posX) {
                this.direction = 0;
                if (this.mouth === 0) {
                    this.mouth = 85;
                } else {this.mouth = 0}
    ...
    moveDown() {
        ...
        if (entity.type === 'wall') {
            ...
            if (this.stage.height - 85 > this.posY) {
                this.direction = -170;
                if (this.mouth === 0) {
                    this.mouth = 85;
                } else {this.mouth = 0}
    ...
    collision(x, y) {
        let entityObj = {};
        let indexOut;
        this.stage.entities.forEach((entity, index) => {
        ...
        if (entityObj.type === 'apple') {
            this.stage.entities.splice(indexOut,1);
            entityObj.element.remove();
            this.stage.score++;
            this.stage.amountOfApples--;

        } else if (entityObj.type === 'bomb') {
            this.stage.entities.splice(indexOut,1);
            entityObj.element.remove();
            if (Math.random() < 0.5 && !this.stage.cheatMode) {
                this.gameOver = true;
    ...
    update () {
        ...
        scoreElm.textContent = `Score: ${this.stage.score}`;
        if (this.stage.amountOfApples === 0) {
            scoreElm.textContent = `Winner`;
            this.stage.element.remove();
